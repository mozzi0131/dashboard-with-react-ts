...
interface UnitTestCol extends Column {
    id: 'module' | 'passRate' | 'utNum' | 'failNum' | 'errNum' | 'memoryLeak';
}
...
function createUnitTestData(module: string, utNum: string, failNum: string, errNum: string, memoryLeak:string): UnitTestData {
    const passRate = returnPassRate(+utNum, +failNum);
    return { module, passRate, utNum, failNum, errNum, memoryLeak};
}
...
export async function setColumnData() {
    ...
    for (const data of moduleData) {
        ...
        const valgrindReport = data.valgrind['error'];

        totalRows.push(createTotalData(moduleName, tempUTnum, tempFailure, lineCover, lineRate, lineValid));
        unittestRows.push(createUnitTestData(moduleName, tempUTnum, tempFailure, tempErr, valgrindReport));
